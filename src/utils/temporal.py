...
def get_optimistic_rounding(
    ...
    if temporal_resolution == TemporalResolution.DAILY:
        ...
        next_month = end_date.replace(
            day=1, month=end_date.month % 12 + 1, tzinfo=datetime.timezone.utc
        )
...
def last_time_of_month(dt: datetime) -> datetime:
    ...
    if dt.month == 12:
        next_month = datetime(dt.year + 1, 1, 1, tzinfo=datetime.timezone.utc)
    else:
        next_month = datetime(dt.year, dt.month + 1, 1, tzinfo=datetime.timezone.utc)
